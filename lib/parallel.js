(function () {
  ...
  Parallel.prototype.getWorkerSource = function (cb, env) {
    ...
    if (isNode) {
      ...
      var outString = '{0};\nself.onmessage = function(e) {\n  var global = self.global || {}; \n  global.{1} = {2};\n  \n  var fn = {3};\n  var asyncFunc = fn.length === 2;\n  \n  var callback = function (response) {\n    self.postMessage(response);\n  };\n\n  var ret = fn(e.data, callback);\n  \n  if (!asyncFunc) {\n    callback(ret);\n  }\n};';
      return stringFormat(outString, preStr, ns, env, cb.toString());
    ...
    if (isNode) {
      ...
      try {
        if (this.requiredScripts.length !== 0) {
            ...
            var url = this._blobUrl = URL.createObjectURL(blob);
  ...
  Parallel.prototype.destroyBlobUrl = function () {
    URL.revokeObjectURL && URL.revokeObjectURL(this._blobUrl);
  };
